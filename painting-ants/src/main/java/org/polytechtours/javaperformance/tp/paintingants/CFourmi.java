...
public class CFourmi {
	...
	*/
	public CFourmi(Color pCouleurDeposee, Color pCouleurSuivie, float pProbaTD, float pProbaG, float pProbaD,
		...
		mLuminanceCouleurSuivie = 0.2426f * pCouleurDeposee.getRed() 
								+ 0.7152f * pCouleurDeposee.getGreen()
								+ 0.0722f * pCouleurDeposee.getBlue();

		// Association avec le CPainting
		mPainting = pPainting;
		
		// Association avec l'application
		mApplis = pApplis;
		...
		mProba[0] = pProbaG; // Gauche
		mProba[1] = pProbaTD; // Tout droit
		mProba[2] = pProbaD; // Droite
		mProba[3] = pProbaSuivre; // Suivre la couleur

		// Nombre de directions pouvant être prises : 
		// 2 types de déplacement possibles
	...
	public synchronized void deplacer() {
		...
		// couleur à suivre, et 1 sinon (dir[0]=gauche, dir[1]=tt_droit, dir[2]=droite)
